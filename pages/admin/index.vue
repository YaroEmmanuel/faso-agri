      ...
      <Transition name="slide">
        ...
        >
          ...
          <div class="flex items-center gap-2 px-6 py-4 border-t border-gray-100 flex-shrink-0">
            <NuxtLink :to="drawer.sectionLink" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50 transition">
            ...
            <div class="flex-1" />
            <!-- Bouton Modifier : navigue vers la section avec ?editId= -->
            <NuxtLink
              v-if="drawer.data"
              :to="drawerEditLink"
              class="inline-flex items-center gap-1.5 px-3 py-2 text-sm border border-blue-200 rounded-xl text-blue-600 hover:bg-blue-50 transition"
            >
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              @click="toggleClose"
              class="inline-flex items-center gap-1.5 px-3 py-2 text-sm border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50 transition"
            >
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path v-if="drawer.data?.status === 'closed'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 11V7a4 4 0 118 0m-4 8v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2z"/>
            ...
            <button
              ...
              class="inline-flex items-center gap-1.5 px-3 py-2 text-sm border border-red-200 rounded-xl text-red-500 hover:bg-red-50 transition"
            >
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const SECTION_CONFIG: Record<string, { label: string; link: string; editLink: string; collections: string[] }> = {
  practical_infos: { label: 'Infos Pratiques', link: '/admin/publications', editLink: '/admin/publications',     collections: ['practical_infos'] },
  discussions:     { label: 'Forum',            link: '/admin/forum',        editLink: '/admin/forum',             collections: ['discussions'] },
  products:        { label: 'Marché agricole',  link: '/admin/catalog',      editLink: '/admin/catalog?tab=products',     collections: ['products'] },
  announcements:   { label: 'Événements',       link: '/admin/catalog',      editLink: '/admin/catalog?tab=announcements', collections: ['announcements'] },
}

// Lien d'édition direct : navigue vers la section avec ?editId= pour auto-ouvrir l'éditeur
const drawerEditLink = computed(() => {
  if (!drawer.data?.id) return drawer.sectionLink
  const cfg = SECTION_CONFIG[drawer.key]
  if (!cfg) return drawer.sectionLink
  const sep = cfg.editLink.includes('?') ? '&' : '?'
  return `${cfg.editLink}${sep}editId=${drawer.data.id}`
})
