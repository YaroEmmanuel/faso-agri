...
import { orderBy, limit, startAfter, query, collection, getDocs } from 'file://C:/Users/emmay/Documents/GitHub/extra/faso-agri-nuxt/node_modules/firebase/firestore/dist/index.mjs';
          ...
          if (_push2) {
            ...
            if (unref(loading)) {
              ...
              ssrRenderList(unref(filteredUsers), (user) => {
                _push2(`<tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition"${_scopeId}><td class="px-5 py-3.5"${_scopeId}><div class="flex items-center gap-3"${_scopeId}><div class="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center text-xs font-semibold flex-shrink-0"${_scopeId}>${ssrInterpolate(initials(user.displayName))}</div><div${_scopeId}><p class="font-medium text-gray-900"${_scopeId}>${ssrInterpolate(user.displayName)}</p><p class="text-xs text-gray-400"${_scopeId}>${ssrInterpolate(user.email)}</p></div></div></td><td class="px-5 py-3.5 text-gray-500 hidden md:table-cell"${_scopeId}>${ssrInterpolate(user.region || "\u2014")}</td><td class="px-5 py-3.5 text-gray-400 hidden lg:table-cell"${_scopeId}>${ssrInterpolate(formatDate(user.createdAt))}</td><td class="px-5 py-3.5"${_scopeId}><span class="${ssrRenderClass([user.status === "active" ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500", "text-xs px-2.5 py-1 rounded-full font-medium"])}"${_scopeId}>${ssrInterpolate(user.status === "active" ? "Actif" : "D\xE9sactiv\xE9")}</span></td><td class="px-5 py-3.5 text-right"${_scopeId}>`);
              ...
              if (unref(filteredUsers).length === 0) {
                _push2(`<tr${_scopeId}><td colspan="5" class="px-5 py-10 text-center text-sm text-gray-400"${_scopeId}> Aucun utilisateur trouv\xE9. </td></tr>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</tbody></table>`);
...
//# sourceMappingURL=index-D8Ct62as.mjs.map
