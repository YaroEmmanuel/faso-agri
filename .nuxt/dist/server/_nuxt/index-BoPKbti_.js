...
import { defineComponent, ref, computed, reactive, withCtx, unref, createVNode, openBlock, createBlock, createTextVNode, withDirectives, isRef, vModelSelect, Fragment, renderList, createCommentVNode, toDisplayString, Teleport, withModifiers, vModelText, useSSRContext } from "vue";
import { ssrRenderComponent, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderList, ssrRenderClass, ssrInterpolate, ssrRenderTeleport, ssrRenderAttr } from "vue/server-renderer";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    async function save(status) {
      ...
      saving.value = true;
      try {
        const payload = { ...form, status, ...status === "published" ? { publishedAt: /* @__PURE__ */ new Date() } : {} };
        if ($firestore) {
          ...
          if (editingId.value) {
            ...
            const ref2 = await addDoc(collection(fs, "publications"), { ...payload, createdAt: serverTimestamp(), createdBy: "admin" });
          ...
          if (editingId.value) {
            ...
            pubs.value = [{ id: String(Date.now()), ...payload, createdAt: /* @__PURE__ */ new Date() }, ...pubs.value];
          }
        }
    ...
    function typeBadge(t) {
      const m = { news: "bg-blue-50 text-blue-600", advice: "bg-amber-50 text-amber-700", alert: "bg-red-50 text-red-600", announcement: "bg-green-50 text-green-700" };
      return m[t] ?? "bg-gray-100 text-gray-500";
    }
    function typeLabel(t) {
      const m = { news: "Actualité", advice: "Conseil", alert: "Alerte", announcement: "Annonce" };
      return m[t] ?? t;
    }
    function statusBadge(s) {
      const m = { published: "bg-green-50 text-green-700", draft: "bg-gray-100 text-gray-500", archived: "bg-gray-100 text-gray-400" };
      return m[s] ?? "bg-gray-100 text-gray-400";
    }
    function statusLabel(s) {
      const m = { published: "Publié", draft: "Brouillon", archived: "Archivé" };
      return m[s] ?? s;
    }
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_AdminLayout, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="space-y-5"${_scopeId}><div class="flex items-center justify-between"${_scopeId}><div${_scopeId}><h1 class="text-xl font-bold text-gray-900"${_scopeId}>Publications</h1><p class="text-sm text-gray-400 mt-0.5"${_scopeId}>Contenus éditoriaux et notifications</p></div><button class="inline-flex items-center gap-2 bg-primary text-white text-sm px-4 py-2 rounded-xl hover:opacity-90 transition"${_scopeId}><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"${_scopeId}></path></svg> Nouvelle publication </button></div><div class="flex flex-wrap gap-3"${_scopeId}><select class="px-4 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition"${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(filterType)) ? ssrLooseContain(unref(filterType), "") : ssrLooseEqual(unref(filterType), "")) ? " selected" : ""}${_scopeId}>Tous les types</option><option value="news"${ssrIncludeBooleanAttr(Array.isArray(unref(filterType)) ? ssrLooseContain(unref(filterType), "news") : ssrLooseEqual(unref(filterType), "news")) ? " selected" : ""}${_scopeId}>Actualité</option><option value="advice"${ssrIncludeBooleanAttr(Array.isArray(unref(filterType)) ? ssrLooseContain(unref(filterType), "advice") : ssrLooseEqual(unref(filterType), "advice")) ? " selected" : ""}${_scopeId}>Conseil agricole</option><option value="alert"${ssrIncludeBooleanAttr(Array.isArray(unref(filterType)) ? ssrLooseContain(unref(filterType), "alert") : ssrLooseEqual(unref(filterType), "alert")) ? " selected" : ""}${_scopeId}>Alerte</option><option value="announcement"${ssrIncludeBooleanAttr(Array.isArray(unref(filterType)) ? ssrLooseContain(unref(filterType), "announcement") : ssrLooseEqual(unref(filterType), "announcement")) ? " selected" : ""}${_scopeId}>Annonce</option></select><select class="px-4 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition"${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(filterStatus)) ? ssrLooseContain(unref(filterStatus), "") : ssrLooseEqual(unref(filterStatus), "")) ? " selected" : ""}${_scopeId}>Tous les statuts</option><option value="draft"${ssrIncludeBooleanAttr(Array.isArray(unref(filterStatus)) ? ssrLooseContain(unref(filterStatus), "draft") : ssrLooseEqual(unref(filterStatus), "draft")) ? " selected" : ""}${_scopeId}>Brouillon</option><option value="published"${ssrIncludeBooleanAttr(Array.isArray(unref(filterStatus)) ? ssrLooseContain(unref(filterStatus), "published") : ssrLooseEqual(unref(filterStatus), "published")) ? " selected" : ""}${_scopeId}>Publié</option><option value="archived"${ssrIncludeBooleanAttr(Array.isArray(unref(filterStatus)) ? ssrLooseContain(unref(filterStatus), "archived") : ssrLooseEqual(unref(filterStatus), "archived")) ? " selected" : ""}${_scopeId}>Archivé</option></select></div><div class="space-y-3"${_scopeId}>`);
            if (unref(loading)) {
            ...
            ssrRenderList(unref(filteredPubs), (pub) => {
              ...
              if (pub.sendNotification) {
                _push2(`<span class="inline-flex items-center gap-1 text-xs text-gray-400"${_scopeId}><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"${_scopeId}></path></svg> Notif. FCM </span>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div><p class="font-medium text-gray-900 line-clamp-1"${_scopeId}>${ssrInterpolate(pub.title)}</p><p class="text-xs text-gray-400 mt-0.5"${_scopeId}>${ssrInterpolate(pub.targetRegion === "all" ? "Diffusion nationale" : pub.targetRegion)} · ${ssrInterpolate(formatDate(pub.createdAt))}</p></div><div class="flex items-center gap-1 flex-shrink-0"${_scopeId}><button title="Modifier" class="p-2 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition"${_scopeId}><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"${_scopeId}></path></svg></button><button title="Archiver" class="p-2 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-red-500 transition"${_scopeId}><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"${_scopeId}></path></svg></button></div></div></div>`);
            });
            _push2(`<!--]-->`);
            if (!unref(loading) && unref(filteredPubs).length === 0) {
              _push2(`<p class="text-center text-sm text-gray-400 py-10"${_scopeId}>Aucune publication trouvée.</p>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div></div>`);
              ...
              createVNode("div", { class: "space-y-5" }, [
                createVNode("div", { class: "flex items-center justify-between" }, [
                  createVNode("div", null, [
                    createVNode("h1", { class: "text-xl font-bold text-gray-900" }, "Publications"),
                    createVNode("p", { class: "text-sm text-gray-400 mt-0.5" }, "Contenus éditoriaux et notifications")
                  ]),
                  createVNode("button", {
                ...
                createVNode("div", { class: "flex flex-wrap gap-3" }, [
                  withDirectives(createVNode("select", {
                    ...
                    createVNode("option", { value: "news" }, "Actualité"),
                ...
                createVNode("div", { class: "space-y-3" }, [
                        ...
                        createVNode("div", { class: "flex-1 min-w-0" }, [
                          createVNode("div", { class: "flex items-center gap-2 mb-1.5 flex-wrap" }, [
                            ...
                            pub.sendNotification ? (openBlock(), createBlock("span", {
                              ...
                              createTextVNode(" Notif. FCM ")
                            ])) : createCommentVNode("", true)
                          ]),
                          createVNode("p", { class: "font-medium text-gray-900 line-clamp-1" }, toDisplayString(pub.title), 1),
                          createVNode("p", { class: "text-xs text-gray-400 mt-0.5" }, toDisplayString(pub.targetRegion === "all" ? "Diffusion nationale" : pub.targetRegion) + " · " + toDisplayString(formatDate(pub.createdAt)), 1)
                        ]),
                  ...
                  }, "Aucune publication trouvée.")) : createCommentVNode("", true)
                ])
              ]),
                  ...
                  createVNode("div", { class: "bg-white rounded-2xl w-full max-w-xl shadow-2xl max-h-[90vh] flex flex-col" }, [
                    ...
                    createVNode("div", { class: "overflow-y-auto flex-1 px-6 py-5 space-y-4" }, [
                      ...
                      createVNode("div", { class: "grid grid-cols-2 gap-3" }, [
                        ...
                        createVNode("div", null, [
                          createVNode("label", { class: "block text-xs font-medium text-gray-500 mb-1.5 uppercase tracking-wide" }, "Région cible"),
                          withDirectives(createVNode("select", {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/admin/publications/index.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
export {
  _sfc_main as default
};
//# sourceMappingURL=index-BoPKbti_.js.map
