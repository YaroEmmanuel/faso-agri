...
import { defineComponent, ref, computed, withCtx, unref, createVNode, toDisplayString, openBlock, createBlock, createTextVNode, withDirectives, isRef, vModelText, Fragment, renderList, vModelSelect, createCommentVNode, useSSRContext } from "vue";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const fallbackUsers = Array.from({ length: 12 }, (_, i) => ({
      id: String(i),
      displayName: ["Moussa Traoré", "Aïcha Koné", "Ibrahim Sawadogo", "Fatou Diallo", "Adama Ouédraogo", "Mariam Coulibaly", "Seydou Kaboré", "Aminata Tapsoba", "Boubacar Zongo", "Kadiatou Bâ", "Lassina Dao", "Rokia Sore"][i],
          ...
          if (_push2) {
            ...
            if (unref(loading)) {
              ...
              ssrRenderList(unref(filteredUsers), (user) => {
                _push2(`<tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition"${_scopeId}><td class="px-5 py-3.5"${_scopeId}><div class="flex items-center gap-3"${_scopeId}><div class="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center text-xs font-bold flex-shrink-0"${_scopeId}>${ssrInterpolate(initials(user.displayName))}</div><div${_scopeId}><p class="font-medium text-gray-900"${_scopeId}>${ssrInterpolate(user.displayName)}</p><p class="text-xs text-gray-400"${_scopeId}>${ssrInterpolate(user.email)}</p></div></div></td><td class="px-5 py-3.5 text-gray-500 hidden md:table-cell"${_scopeId}>${ssrInterpolate(user.region || "—")}</td><td class="px-5 py-3.5 text-gray-400 hidden lg:table-cell"${_scopeId}>${ssrInterpolate(formatDate(user.createdAt))}</td><td class="px-5 py-3.5"${_scopeId}><span class="${ssrRenderClass([user.status === "active" ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500", "text-xs px-2.5 py-1 rounded-full font-medium"])}"${_scopeId}>${ssrInterpolate(user.status === "active" ? "Actif" : "Désactivé")}</span></td></tr>`);
              });
              _push2(`<!--]-->`);
              if (unref(filteredUsers).length === 0) {
                _push2(`<tr${_scopeId}><td colspan="4" class="px-5 py-10 text-center text-sm text-gray-400"${_scopeId}>Aucun utilisateur trouvé.</td></tr>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</tbody></table>`);
...
//# sourceMappingURL=index-V--j80yl.js.map
