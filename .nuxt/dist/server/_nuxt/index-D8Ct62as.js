...
import { _ as __nuxt_component_0 } from "./nuxt-link-CkXNCsDJ.js";
import { defineComponent, ref, computed, withCtx, unref, createTextVNode, createVNode, toDisplayString, openBlock, createBlock, withDirectives, isRef, vModelText, Fragment, renderList, vModelSelect, createCommentVNode, useSSRContext } from "vue";
...
import { orderBy, limit, startAfter, query, collection, getDocs } from "firebase/firestore";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_AdminLayout, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="space-y-5"${_scopeId}><div class="flex items-center justify-between"${_scopeId}><div${_scopeId}><h1 class="text-xl font-semibold text-gray-900"${_scopeId}>Utilisateurs</h1><p class="text-sm text-gray-400 mt-0.5"${_scopeId}>${ssrInterpolate(unref(totalCount))} comptes enregistrés</p></div><button class="flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900 border border-gray-200 rounded-xl px-4 py-2 hover:border-gray-300 transition"${_scopeId}><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"${_scopeId}></path></svg> Exporter CSV </button></div><div class="flex flex-wrap gap-3"${_scopeId}><input${ssrRenderAttr("value", unref(search))} type="text" placeholder="Rechercher par nom ou email…" class="flex-1 min-w-52 px-4 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition"${_scopeId}><select class="px-4 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition"${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(filterRegion)) ? ssrLooseContain(unref(filterRegion), "") : ssrLooseEqual(unref(filterRegion), "")) ? " selected" : ""}${_scopeId}>Toutes les régions</option><!--[-->`);
            ...
            _push2(`<!--]--></select><select class="px-4 py-2 text-sm rounded-xl border border-gray-200 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition"${_scopeId}><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(filterStatus)) ? ssrLooseContain(unref(filterStatus), "") : ssrLooseEqual(unref(filterStatus), "")) ? " selected" : ""}${_scopeId}>Tous les statuts</option><option value="active"${ssrIncludeBooleanAttr(Array.isArray(unref(filterStatus)) ? ssrLooseContain(unref(filterStatus), "active") : ssrLooseEqual(unref(filterStatus), "active")) ? " selected" : ""}${_scopeId}>Actif</option><option value="disabled"${ssrIncludeBooleanAttr(Array.isArray(unref(filterStatus)) ? ssrLooseContain(unref(filterStatus), "disabled") : ssrLooseEqual(unref(filterStatus), "disabled")) ? " selected" : ""}${_scopeId}>Désactivé</option></select></div><div class="bg-white rounded-2xl border border-gray-100 overflow-hidden"${_scopeId}>`);
            if (unref(loading)) {
              ...
              _push2(`<table class="w-full text-sm"${_scopeId}><thead${_scopeId}><tr class="border-b border-gray-100 text-xs text-gray-400 uppercase tracking-wide"${_scopeId}><th class="text-left px-5 py-3 font-medium"${_scopeId}>Utilisateur</th><th class="text-left px-5 py-3 font-medium hidden md:table-cell"${_scopeId}>Région</th><th class="text-left px-5 py-3 font-medium hidden lg:table-cell"${_scopeId}>Inscription</th><th class="text-left px-5 py-3 font-medium"${_scopeId}>Statut</th><th class="px-5 py-3"${_scopeId}></th></tr></thead><tbody${_scopeId}><!--[-->`);
              ssrRenderList(unref(filteredUsers), (user) => {
                _push2(`<tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition"${_scopeId}><td class="px-5 py-3.5"${_scopeId}><div class="flex items-center gap-3"${_scopeId}><div class="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center text-xs font-semibold flex-shrink-0"${_scopeId}>${ssrInterpolate(initials(user.displayName))}</div><div${_scopeId}><p class="font-medium text-gray-900"${_scopeId}>${ssrInterpolate(user.displayName)}</p><p class="text-xs text-gray-400"${_scopeId}>${ssrInterpolate(user.email)}</p></div></div></td><td class="px-5 py-3.5 text-gray-500 hidden md:table-cell"${_scopeId}>${ssrInterpolate(user.region || "—")}</td><td class="px-5 py-3.5 text-gray-400 hidden lg:table-cell"${_scopeId}>${ssrInterpolate(formatDate(user.createdAt))}</td><td class="px-5 py-3.5"${_scopeId}><span class="${ssrRenderClass([user.status === "active" ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500", "text-xs px-2.5 py-1 rounded-full font-medium"])}"${_scopeId}>${ssrInterpolate(user.status === "active" ? "Actif" : "Désactivé")}</span></td><td class="px-5 py-3.5 text-right"${_scopeId}>`);
              ...
              if (unref(filteredUsers).length === 0) {
                _push2(`<tr${_scopeId}><td colspan="5" class="px-5 py-10 text-center text-sm text-gray-400"${_scopeId}> Aucun utilisateur trouvé. </td></tr>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</tbody></table>`);
            }
            _push2(`</div><div class="flex items-center justify-between text-sm text-gray-400"${_scopeId}><span${_scopeId}>Page ${ssrInterpolate(unref(page))} · ${ssrInterpolate(unref(filteredUsers).length)} résultats</span><div class="flex gap-2"${_scopeId}><button${ssrIncludeBooleanAttr(unref(page) === 1) ? " disabled" : ""} class="px-4 py-1.5 rounded-xl border border-gray-200 hover:border-gray-300 disabled:opacity-40 disabled:cursor-not-allowed transition text-gray-600"${_scopeId}> Précédent </button><button${ssrIncludeBooleanAttr(!unref(hasMore)) ? " disabled" : ""} class="px-4 py-1.5 rounded-xl border border-gray-200 hover:border-gray-300 disabled:opacity-40 disabled:cursor-not-allowed transition text-gray-600"${_scopeId}> Suivant </button></div></div></div>`);
          } else {
            return [
              createVNode("div", { class: "space-y-5" }, [
                createVNode("div", { class: "flex items-center justify-between" }, [
                  createVNode("div", null, [
                    createVNode("h1", { class: "text-xl font-semibold text-gray-900" }, "Utilisateurs"),
                    createVNode("p", { class: "text-sm text-gray-400 mt-0.5" }, toDisplayString(unref(totalCount)) + " comptes enregistrés", 1)
                  ]),
                  createVNode("button", {
                    onClick: exportCsv,
                ...
                createVNode("div", { class: "flex flex-wrap gap-3" }, [
                  ...
                  withDirectives(createVNode("select", {
                    ...
                    createVNode("option", { value: "" }, "Toutes les régions"),
                  ...
                  unref(loading) ? (openBlock(), createBlock("div", {
                    ...
                    createVNode("thead", null, [
                      createVNode("tr", { class: "border-b border-gray-100 text-xs text-gray-400 uppercase tracking-wide" }, [
                        ...
                        createVNode("th", { class: "text-left px-5 py-3 font-medium hidden md:table-cell" }, "Région"),
                        ...
                        return openBlock(), createBlock("tr", {
                          ...
                          createVNode("td", { class: "px-5 py-3.5" }, [
                            createVNode("div", { class: "flex items-center gap-3" }, [
                              createVNode("div", { class: "w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center text-xs font-semibold flex-shrink-0" }, toDisplayString(initials(user.displayName)), 1),
                              createVNode("div", null, [
                          ...
                          createVNode("td", { class: "px-5 py-3.5" }, [
                            ...
                            }, toDisplayString(user.status === "active" ? "Actif" : "Désactivé"), 3)
                          ]),
                ...
                createVNode("div", { class: "flex items-center justify-between text-sm text-gray-400" }, [
                  createVNode("span", null, "Page " + toDisplayString(unref(page)) + " · " + toDisplayString(unref(filteredUsers).length) + " résultats", 1),
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/admin/mnt/user-data/outputs/faso-agri-admin/pages/admin/users/index.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
export {
  _sfc_main as default
};
//# sourceMappingURL=index-D8Ct62as.js.map
